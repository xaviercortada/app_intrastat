...
public class NomenclatureService {
    ...
    public Nomenclature findByCodigo(String codigo){
        TypedQuery<Nomenclature>query = em.createQuery("SELECT p FROM Nomenclature p WHERE p.code = ?0",Nomenclature.class);
    ...
    public  List<Nomenclature> findByLevel(int level){
        TypedQuery<Nomenclature>query = em.createQuery("SELECT p FROM Nomenclature p WHERE p.level = ?0",Nomenclature.class);
   ...
   public List<Nomenclature> findCapitulosBySeccion(String codigo) {
        try{
            ...
            TypedQuery<Nomenclature>query = em.createQuery("SELECT p FROM Nomenclature p WHERE p.level = 2 and p.code between ?0 and ?1",Nomenclature.class);
            query.setParameter(0, codigo);
	...
	public List<Nomenclature> findItemsByText(String codigo, String texto) {
	    try{
	        TypedQuery<Nomenclature>query = em.createQuery("SELECT p FROM Nomenclature p WHERE p.level > 2 and p.code like ?0 and description like ?1",Nomenclature.class);
	        query.setParameter(0, codigo+'%');
	...
	public Long countItems(String texto) {
	    try{
	        Query query = em.createQuery("SELECT COUNT(p) FROM Nomenclature p WHERE p.level > 2 and description like ?0");
    ...
    try{
        TypedQuery<Nomenclature>query = em.createQuery("SELECT p FROM Nomenclature p WHERE p.level > 2 and description like ?0",Nomenclature.class);
        query.setParameter(0, '%'+texto+'%');
