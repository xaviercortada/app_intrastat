...
public class FacturaService {
    ...
    public List<Factura> findAll(final Account account) {
        final TypedQuery<Factura> query = (TypedQuery<Factura>) this.em
                .createQuery("SELECT p FROM Factura p WHERE p.account.id = ?0 and p.company.id = ?1", Factura.class);
    ...
    public List<Factura> findByUser(final Account account) {
        final TypedQuery<Factura> query = (TypedQuery<Factura>) this.em
                .createQuery("SELECT p FROM Factura p WHERE p.account.id = ?0", Factura.class);
        query.setParameter(0, (Object) account.getId());
        return (List<Factura>) query.getResultList();
    }
    ...
    public List<Factura> findByIntervalo(final Account account, final String flujo, final String present,
            final Date fechaIni, final Date fechaFin) {
        String s = this.checkPresent(present);
        s = String.valueOf(s) + this.checkFlujo(flujo);
        final TypedQuery<Factura> query = (TypedQuery<Factura>) this.em.createQuery(
                "SELECT p FROM Factura p WHERE fecha >= ?0 and fecha < ?1 and p.account.id = ?2" + s, Factura.class);
    ...
    public List<Factura> findByProveedor(final Account account, final String flujo, final String present,
            final Long idProveedor) {
        String s = this.checkPresent(present);
        s = String.valueOf(s) + this.checkFlujo(flujo);
        // final TypedQuery<Factura> query =
        // (TypedQuery<Factura>)this.em.createQuery("SELECT p FROM Factura p WHERE
        // p.proveedor.id = ?0 and p.account.id = ?1 and p.company.id = ?2" + s,
        // Factura.class);
        final TypedQuery<Factura> query = (TypedQuery<Factura>) this.em.createQuery(
                "SELECT p FROM Factura p WHERE p.proveedor.id = ?0 and p.account.id = ?1" + s, Factura.class);
    ...
    public List<Factura> findPendientes() {
        final TypedQuery<Factura> query = (TypedQuery<Factura>) this.em
                .createQuery("SELECT p FROM Factura p WHERE p.presentado IS NULL", Factura.class);
        return (List<Factura>) query.getResultList();
    }
    ...
    public List<Factura> findByCodigo(final Account account, final String codigo) {
        // final TypedQuery<Factura> query =
        // (TypedQuery<Factura>)this.em.createQuery("SELECT p FROM Factura p WHERE
        // p.account.id = ?0 and p.company.id = ?1 and p.codigo = ?2", Factura.class);
        final TypedQuery<Factura> query = (TypedQuery<Factura>) this.em
                .createQuery("SELECT p FROM Factura p WHERE p.account.id = ?0 and p.codigo = ?1", Factura.class);
        query.setParameter(0, (Object) account.getId());
        // query.setParameter(1, (Object)account.getActiveCompany());
    ...
    public List<Factura> findByState(final Account account, final String flujo, final String present) {
        ...
        // final TypedQuery<Factura> query =
        // (TypedQuery<Factura>)this.em.createQuery("SELECT p FROM Factura p WHERE
        // p.account.id = ?0 and p.company.id = ?1 " + s, Factura.class);
        final TypedQuery<Factura> query = (TypedQuery<Factura>) this.em
                .createQuery("SELECT p FROM Factura p WHERE p.account.id = ?0 " + s, Factura.class);
        query.setParameter(0, (Object) account.getId());
        // query.setParameter(1, (Object)account.getActiveCompany());
        return (List<Factura>) query.getResultList();
    }
